S= [0 0.5 1 1.5 2 2.5 3 3.5 4 4.5 5 5.5 6];
kc=3;
...
for j = 1:length(S)
    ...
    for k = 1:length(fr);
        ...
        [Xdet{j,k,l},Xsto{j,k,l},~,Po1{j,k,l},Cdet1{j,k,l}]=hbsimplemodel(0,kc,S(j),0.1,Fextmax(l),fr(k),t);
        end
    end
end
%}
...
for j = 1:sizeX(1)
    for k = 1:sizeX(2)
        for l = 1:sizeX(3)
                ...
                trsto=Xsto{j,k,l}(1,round(L/4):end)-smooth(Xsto{j,k,l}(1,round(L/4):end),length(Xsto{j,k,l}(1,round(L/4):end))/4)';
                ...
                fstoind = findnearest(fstofft ,fr(k));fstoind=fstoind(1);
                ...
                Xstofftmaxind = fstoind;
                 if isempty(Xstofftmaxind) ==0
                fftamplsto(j,k,l) = Xstofft(fstoind);
                fftamplsto(j,k,l) = (sqrt(fscale.*fftamplsto(j,k,l).*(2.*Fs.*XsegL.*(sum(abs(winfunc).^2)./XsegL)))./XsegL)./winpeaknorm;        
                fftfreqsto(j,k,l) = fstofft(fstoind);           
                if  fftamplsto(j,k,l) < 1e-3
                    fftfreqsto(j,k,l) = 0;
                    fftamplsto(j,k,l) = 0;
                end
                 else
                ...
                if  fftampldet(j,k,l) < 1e-3
                 ...
                 meanPodet(j,k,l) = mean(Po1(round(length(Po)/2):end));
                 stdPodet(j,k,l)=std(Po1(round(length(Po)/2):end));
...
for j = 1:sizeX(3)
    figure(j+sizeX(3));
    for k = 1:sizeX(1)
        errorbar(fr,meanPodet(k,:,j),stdPodet(k,:,j));
...
for j = 1:sizeX(3)
    figure(j+2*sizeX(3));
    for k = 1:sizeX(1)
        errorbar(fr,meanCdet(k,:,j),stdCdet(k,:,j));
