function [Xdet, Xsto, Fext] = noiseEM2(I,thetaNoiseSTD,tvec)
%
...
for j = 2:N
%Stochastic integral:last term only
xdet(j) = xsto(j-1) + thetaNoiseSTD*(1 + cos(xdet(j-1)))*dW(j);

%Stochastic integral:complete
%xsto(j) = xsto(j-1) + Dt*(1 - cos(xsto(j-1)) + (1 + cos(xsto(j-1)))*I + Fext(j)) + thetaNoiseSTD*dW(j);
xsto(j) = xsto(j-1) + Dt*((1 + cos(xsto(j-1)))*(I - thetaNoiseSTD^2/2*sin(xsto(j-1)))) + thetaNoiseSTD*(1 + cos(xsto(j-1)))*dW(j);
