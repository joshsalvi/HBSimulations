function [Xdet Xem] = HBmodel3EM( Time )
%Alternate HB model
...
xdet(j) = xdet(j-1) + Dt*((-k*xdet(j-1) + a*xdet(j-1) - (xdet(j-1)-fdet(j-1))^3 + Fc)/gamma);
fdet(j) = fdet(j-1) + Dt*((b*xdet(j-1) - fdet(j-1))/tau);

%Stochastic integral
xem(j) = xem(j-1) + Dt*((-k*xem(j-1) + a*xem(j-1) - (xem(j-1)-fem(j-1))^3 + Fc)/gamma) + xNoiseSTD*xdW(j);
