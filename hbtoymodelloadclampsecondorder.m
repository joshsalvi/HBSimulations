function [Xdet, Xsto, Deltadet, Deltasto,Fext] = hbtoymodelloadclampsecondorder(Fc,k,noiselevel,Fextmax,fr,ksf,Fe,kv,ev,mv,G,tvec)
%
% This function simulates the hair-buyndle model from PNAS 2012.
%
% [Xdet, Xsto, Delta, Fext2] = hbtoymodelloadclampsecondorder(Fc,k,noiselevel,Fextmax,fr,ksf,Fe,kv,ev,mv,G,tvec)
%
...
Xcdet(j) = (edx*Xcdet(j-1))/((edx-ksf*Dt)*alpha) + Dt*((mv*xdetdd(j)+(ev-exx+alpha*beta*G*edx)*xdetd(j)+(kv-(1+alpha*beta*G)*ksf)*xdet(j-1)-Fe)/(alpha*beta*G*(edx-ksf*Dt)) + real(Fext(j)));
Xcsto(j) = (edx*Xcsto(j-1))/((edx-ksf*Dt)*alpha) + Dt*((mv*xstodd(j)+(ev-exx+alpha*beta*G*edx)*xstod(j)+(kv-(1+alpha*beta*G)*ksf)*xsto(j-1)-Fe)/(alpha*beta*G*(edx-ksf*Dt)) + imag(Fext(j)));
