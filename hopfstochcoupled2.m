function [Xdet, Xsto, Fext] = hopfstochcoupled2(mu,fosc,NoiseSTD,K,tvec,Fextmax,fr)
%
...
xdet(j) = xdet(j-1) + Dt*(mu*xdet(j-1) - 2*pi*fosc*ydet(j-1) - xdet(j-1)*(xdet(j-1)^2 + ydet(j-1)^2) + real(Fext(j)) + K*x2det(j-1));
ydet(j) = ydet(j-1) + Dt*(2*pi*fosc*xdet(j-1) + mu*ydet(j-1) - ydet(j-1)*(xdet(j-1)^2 + ydet(j-1)^2) + imag(Fext(j)) + K*y2det(j-1));
x2det(j) = x2det(j-1) + Dt*(mu*x2det(j-1) - 2*pi*fosc*y2det(j-1) - x2det(j-1)*(x2det(j-1)^2 + y2det(j-1)^2) + real(Fext(j)) + K*xdet(j-1));
y2det(j) = y2det(j-1) + Dt*(2*pi*fosc*x2det(j-1) + mu*y2det(j-1) - y2det(j-1)*(x2det(j-1)^2 + y2det(j-1)^2) + imag(Fext(j)) + K*ydet(j-1));


%Stochastic integral
xsto(j) = xsto(j-1) + Dt*(mu*xsto(j-1) - 2*pi*fosc*ysto(j-1) - xsto(j-1)*(xsto(j-1)^2 + ysto(j-1)^2) + real(Fext(j)) + K*x2sto(j-1)) + xNoiseSTD*xdW(j);
ysto(j) = ysto(j-1) + Dt*(2*pi*fosc*xsto(j-1) + mu*ysto(j-1) - ysto(j-1)*(xsto(j-1)^2 + ysto(j-1)^2) + imag(Fext(j)) + K*y2sto(j-1)) + yNoiseSTD*ydW(j);
x2sto(j) = x2sto(j-1) + Dt*(mu*x2sto(j-1) - 2*pi*fosc*y2sto(j-1) - x2sto(j-1)*(x2sto(j-1)^2 + y2sto(j-1)^2) + real(Fext(j)) + K*xsto(j-1)) + xNoiseSTD*xdW(j);
y2sto(j) = y2sto(j-1) + Dt*(2*pi*fosc*x2sto(j-1) + mu*y2sto(j-1) - y2sto(j-1)*(x2sto(j-1)^2 + y2sto(j-1)^2) + imag(Fext(j)) + K*ysto(j-1)) + yNoiseSTD*ydW(j);
...
Xdet(3,:) = x2det(1:Dtfac:N);
Xdet(4,:) = y2det(1:Dtfac:N);

Xsto(1,:) = xsto(1:Dtfac:N);
Xsto(2,:) = ysto(1:Dtfac:N);
Xsto(3,:) = x2sto(1:Dtfac:N);
Xsto(4,:) = y2sto(1:Dtfac:N);
