...
%
% [Xdet Xsto] = subhopfstochoffset(mu,omega,Co,xNoiseSTD,yNoiseSTD,tvec)
%
...
xdet(j) = xdet(j-1) + Dt*(mu*xdet(j-1) - 2*pi*fosc*ydet(j-1) + xdet(j-1)*(xdet(j-1)^2 + ydet(j-1)^2) - xdet(j-1)*(xdet(j-1)^2 + ydet(j-1)^2)^2 + Fext(j) + Co);
ydet(j) = ydet(j-1) + Dt*(2*pi*fosc*xdet(j-1) + mu*ydet(j-1) + ydet(j-1)*(xdet(j-1)^2 + ydet(j-1)^2) - ydet(j-1)*(xdet(j-1)^2 + ydet(j-1)^2)^2 + Fext(j) + Co);
%xdet(j) = xdet(j-1) + Dt*(mu*xdet(j-1) + xdet(j-1)^3 - xdet(j-1)^5 + abs(Fext(j)) + Co);
%ydet(j) = ydet(j-1) + Dt*(omega + xdet(j-1)^2 + angle(Fext(j)) + Co);

%Stochastic integral
xsto(j) = xsto(j-1) + Dt*(mu*xsto(j-1) - 2*pi*fosc*ysto(j-1) + xsto(j-1)*(xsto(j-1)^2 + ysto(j-1)^2) - xsto(j-1)*(xsto(j-1)^2 + ysto(j-1)^2)^2 + Fext(j) + Co) + xNoiseSTD*xdW(j);
ysto(j) = ysto(j-1) + Dt*(2*pi*fosc*xsto(j-1) + mu*ysto(j-1) + ysto(j-1)*(xsto(j-1)^2 + ysto(j-1)^2) - ysto(j-1)*(xsto(j-1)^2 + ysto(j-1)^2)^2 + Fext(j) + Co) + yNoiseSTD*ydW(j);
%xsto(j) = xsto(j-1) + Dt*(mu*xsto(j-1) + xsto(j-1)^3 - xsto(j-1)^5 + abs(Fext(j)) + Co) + xNoiseSTD*xdW(j);
%ysto(j) = ysto(j-1) + Dt*(omega + xsto(j-1)^2 + angle(Fext(j)) + Co) + yNoiseSTD*ydW(j);
...
if plotyn==1
    ...
    subplot(1,2,2);hold on;plot(Xsto(1,2:end),diff(Xsto(1,:)),'r');plot(Xdet(1,2:end),diff(Xdet(1,:)),'k');title('Black=deterministic; Red=stochastic');
end
