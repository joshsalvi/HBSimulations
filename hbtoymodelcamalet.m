function [Xdet, Xsto, kdet, Fext2] = hbtoymodelcamalet(Fc,kzero,tauk,delta,noiselevel,Fextmax,fr,tvec)
%
...
xdet(j) = xdet(j-1) + Dt*(-kdet(j-1)*xdet(j-1) + a*(xdet(j-1)-fdet(j-1)) - (xdet(j-1)-fdet(j-1))^3 + Fc + Fext(j));
fdet(j) = fdet(j-1) + Dt*(b*xdet(j-1) - fdet(j-1))/tau;
kdet(j) = kdet(j-1) + Dt*(kdet(j-1)/tauk*xdet(j-1)^2/delta^2-kdet(j-1)/tauk);

%Stochastic integral
xsto(j) = xsto(j-1) + Dt*(-ksto(j-1)*xsto(j-1) + a*(xsto(j-1)-fsto(j-1)) - (xsto(j-1)-fsto(j-1))^3 + Fc + Fext(j)) + xNoiseSTD*xdW(j);
fsto(j) = fsto(j-1) + Dt*(b*xsto(j-1) - fsto(j-1))/tau + fNoiseSTD*fdW(j)/tau;
ksto(j) = ksto(j-1) + Dt*(ksto(j-1)/tauk*xsto(j-1)^2/delta^2-ksto(j-1)/tauk) + kNoiseSTD*kdW(j)/tauk;
end
